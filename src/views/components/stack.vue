  ...
  methods: {
    ...
    touchmove(e) {
      ...
      if (this.temporaryData.tracking && !this.temporaryData.animation) {
        ...
        if (
          (topNum < 0 &&
            Math.round(Math.abs(topNum)) > Math.round(dragBoxHeight / 2)) ||
          (topBottom > 0 &&
            Math.round(Math.abs(topBottom)) > Math.round(dragBoxHeight / 2))
        ) {
          let dragWidthNum =
            (e.type === "touchmove" ? e.targetTouches[0].clientX : e.clientX) -
            this.basicdata.start.x;
