	...
	methods: {
		...
		touchmove(e) {
			...
			if (this.temporaryData.tracking && !this.temporaryData.animation) {
				...
				// 纵向拖动的距离
				let dragHeightDiff = (e.type === "touchmove" ? e.targetTouches[0].clientY : e.clientY) - this.basicdata.start.y;
				// 盒子的高度
				...
				// 计算盒子拖动后超过底部的距离
				let topBottom = dragTopNum + dragHeightDiff + dragBoxHeight - clientHeight;
				// 限制盒子往上、往下只能拖动二分之一的距离
				if (
					(topNum < 0 && Math.round(Math.abs(topNum)) > Math.round(dragBoxHeight / 2)) ||
				...
				this.$emit("update:like", this.temporaryData.poswidth < 0 ? false : true);
				this.$emit(
					"change",
					"progress",
		...
		nextTick() {
			...
			// 处理获取喜欢、不喜欢的值
			console.log(this.temporaryData.lastRotate, "获取偏移值", this.temporaryData.lastPosWidth);
